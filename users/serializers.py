...
class UserCreateSerializer(serializers.ModelSerializer):
    member_type = serializers.PrimaryKeyRelatedField(
        many=False, queryset=member_types.models.MemberType.objects.all())
    ...
    def validate(self, attrs):
        if User.objects.filter(email=attrs['email']).exists():
            raise serializers.ValidationError(
                {'email': "user with email({}) already exists".format(attrs['email'])})
    ...
    def create(self, validated_data):
        ...
        member = members.models.Member(
            member_type=member_type, user=created_user, **validated_data)
        member.save()
        ...
        message = EmailMultiAlternatives(
            subject, text_content, EMAIL_HOST_USER, [created_user.email])
        ...
        r = requests.post(settings.MFS_ENDPOINT+"/api/v1/whitelist/opt_in",
...
class CreateIndividualSerializer(UserCreateSerializer):
    member_type = serializers.PrimaryKeyRelatedField(
        many=False, queryset=member_types.models.MemberType.objects.filter(type='Individual'))
...
class CreateBusinessSerializer(UserCreateSerializer):
    member_type = serializers.PrimaryKeyRelatedField(
        many=False, queryset=member_types.models.MemberType.objects.filter(type='Business'))
...
class CreateFutureSerializer(UserCreateSerializer):
    member_type = serializers.PrimaryKeyRelatedField(
        many=False, queryset=member_types.models.MemberType.objects.filter(type='Future'))
